...
def ansi_generate(
    ...
    color_mode = str(getattr(cfg, "color_mode", "truecolor")).lower()
    is_256 = color_mode == "256"

    if mode == "quadrant" or mode == "octant":
        lookup_func = _ensure_octant_lookup if mode == "octant" else _ensure_quadrant_lookup
        lookup_attr = "_octant" if mode == "octant" else "_quadrant"
        if is_256:
            return _ansi_generate_glyph_256(
                xs, ys, colors, byte_vals, byte_lens, cfg,
                lookup_func, lookup_attr,
                run_color_diff_thresh_override=run_color_diff_thresh_override,
            )
        return _ansi_generate_glyph_rgb(
            xs, ys, colors, byte_vals, byte_lens, cfg,
            lookup_func, lookup_attr,
            run_color_diff_thresh_override=run_color_diff_thresh_override,
        )
    if is_256:
        return ansi_generate_256(
            xs,
            ys,
            colors,
            byte_vals,
...
def _ansi_generate_glyph_rgb(
    ...
    """Shared truecolor glyph-mode generator (quadrant / octant)."""
    N = xs.numel()
    if N == 0:
        return torch.tensor(RESET_VALS, dtype=torch.uint8, device=cfg.device)
    ...
    ensure_lookup_fn(cfg, device)

    glyph_lookup_bytes = getattr(cfg, f"{lookup_attr}_lookup_bytes")
    glyph_lookup_lens = getattr(cfg, f"{lookup_attr}_lookup_lens")
    ...
    glyph_lens = glyph_lookup_lens[run_glyph_idx].to(torch.int64)
    ...
    lookup_max_len = byte_vals.size(1)
    glyph_lookup_max_len = glyph_lookup_bytes.size(1)
    grid = (num_runs,)
    block_size = min(lookup_max_len, 64)
...
def _ansi_generate_glyph_256(
    ...
    """Shared 256-color glyph-mode generator (quadrant / octant)."""
    N = xs.numel()
    if N == 0:
        return torch.tensor(RESET_VALS, dtype=torch.uint8, device=cfg.device)
    ...
    run_fg = styles_sorted[:, 0].to(torch.int64)
    run_bg = styles_sorted[:, 1].to(torch.int64)
    run_glyph_idx = styles_sorted[:, 2].to(torch.int64)
    ...
    needs_style_change = torch.ones(num_runs, dtype=torch.bool, device=device)
    if num_runs > 1:
        style_diff = (styles_sorted[1:] != styles_sorted[:-1]).any(dim=1)
        needs_style_change[1:] = style_diff

    row_idx = run_ys + 1
    col_idx = run_xs + 1
    row_lens = byte_lens[row_idx].to(torch.int64)
    col_lens = byte_lens[col_idx].to(torch.int64)
    fg_lens = byte_lens[run_fg].to(torch.int64)
    bg_lens = byte_lens[run_bg].to(torch.int64)
    ...
    h_mask = (move_kind == 2) | (move_kind == 3)
    h_lens_m = byte_lens[move_h[h_mask].to(torch.long)].to(torch.int64)
    move_lens[h_mask] = 2 + h_lens_m + 1

    v_mask = (move_kind == 4) | (move_kind == 5)
    v_lens_m = byte_lens[move_v[v_mask].to(torch.long)].to(torch.int64)
    move_lens[v_mask] = 2 + v_lens_m + 1
    ...
    fg_len_full = 7 + fg_lens + 1
    bg_len_full = 7 + bg_lens + 1
    style_part = (fg_len_full + bg_len_full) * needs_style_change.to(torch.int64)
    ...
    ansi_256_quadrant_kernel[grid](
        ...
        COL_256_PREF_VALS[5],
        COL_256_PREF_VALS[6],
        BG_COL_256_PREF_VALS[0],
        BG_COL_256_PREF_VALS[1],
        BG_COL_256_PREF_VALS[2],
        BG_COL_256_PREF_VALS[3],
        BG_COL_256_PREF_VALS[4],
        BG_COL_256_PREF_VALS[5],
        BG_COL_256_PREF_VALS[6],
        BLOCK_SIZE=block_size,
        GLYPH_BLOCK_SIZE=glyph_lookup_max_len,
        num_warps=num_warps,
    )
...
# ---------------------------------------------------------------------------
#  256-color mode: Triton kernels & generator functions
# ---------------------------------------------------------------------------
...
def ansi_256_run_kernel(
    ...
    color_val = tl.load(run_color_ptr + pid)
    length = tl.load(run_lengths_ptr + pid)
    offset = tl.load(offsets_ptr + pid)
    move_kind = tl.load(move_kind_ptr + pid)
    move_v = tl.load(move_v_ptr + pid)
    ...
    # --- cursor movement (identical to truecolor) ---
    if move_kind != 0:
        if move_kind == 1:
            tl.store(ptr, ESC0)
            ptr += 1
    ...
    if needs_color_change:
        ...
        c_len = tl.load(col_lookup_lens_ptr + color_val)
        c_bytes_base = col_lookup_bytes_ptr + color_val * BLOCK_SIZE
        for i in tl.static_range(BLOCK_SIZE):
            if i < c_len:
                byte = tl.load(c_bytes_base + i)
                tl.store(ptr + i, byte)
        ptr += c_len
        tl.store(ptr, 109)  # 'm'
        ptr += 1

    # --- payload: spaces (or REP) ---
    if use_rep_run:
        tl.store(ptr, SPACE)
        ptr += 1
        tl.store(ptr, ESC0)
...
def ansi_256_quadrant_kernel(
    ...
    fg_val = tl.load(run_fg_ptr + pid)
    bg_val = tl.load(run_bg_ptr + pid)
    glyph_idx = tl.load(glyph_idx_ptr + pid)
    offset = tl.load(offsets_ptr + pid)
    move_kind = tl.load(move_kind_ptr + pid)
    move_v = tl.load(move_v_ptr + pid)
    ...
    if needs_style_change:
        ...
        tl.store(ptr + 6, FG_PREF6)
        ptr += 7
        fg_len = tl.load(fg_lookup_lens_ptr + fg_val)
        fg_bytes_base = fg_lookup_bytes_ptr + fg_val * BLOCK_SIZE
        for i in tl.static_range(BLOCK_SIZE):
            if i < fg_len:
                byte = tl.load(fg_bytes_base + i)
                tl.store(ptr + i, byte)
        ptr += fg_len
        tl.store(ptr, 109)  # 'm'
        ptr += 1
        ...
        tl.store(ptr + 6, BG_PREF6)
        ptr += 7
        bg_len = tl.load(bg_lookup_lens_ptr + bg_val)
        bg_bytes_base = bg_lookup_bytes_ptr + bg_val * BLOCK_SIZE
        for i in tl.static_range(BLOCK_SIZE):
            if i < bg_len:
                byte = tl.load(bg_bytes_base + i)
                tl.store(ptr + i, byte)
        ptr += bg_len
        tl.store(ptr, 109)  # 'm'
        ptr += 1
...
def ansi_generate_256(
    ...
    """Generate ANSI escape sequences using 256-color palette.

    `colors` is a 1-D (N,) uint8 tensor of palette indices.
    """
    N = xs.numel()
    if N == 0:
        return torch.tensor(RESET_VALS, dtype=torch.uint8, device=cfg.device)
    ...
    # --- Run detection (single channel) ---
    y_diff = ys_sorted[1:] != ys_sorted[:-1]
    x_diff = xs_sorted[1:] - xs_sorted[:-1]
    color_diff = colors_sorted[1:] != colors_sorted[:-1]
    ...
    run_color_idx = colors_sorted[run_start_indices].to(torch.int64)

    needs_color_change = torch.ones(num_runs, dtype=torch.bool, device=device)
    if num_runs > 1:
        needs_color_change[1:] = run_color_idx[1:] != run_color_idx[:-1]

    row_idx = run_ys + 1
    col_idx = run_xs + 1
    row_lens = byte_lens[row_idx].to(torch.int64)
    col_lens = byte_lens[col_idx].to(torch.int64)
    color_lens = byte_lens[run_color_idx].to(torch.int64)

    # --- Cursor movement (same as truecolor) ---
    move_kind = torch.zeros(num_runs, dtype=torch.int8, device=device)
    move_v = torch.zeros(num_runs, dtype=torch.int64, device=device)
    move_h = torch.zeros(num_runs, dtype=torch.int64, device=device)
    ...
    # 256-color: \e[48;5;Nm  ->  7 prefix + color_digit_len + 1 suffix
    color_len_full = 7 + color_lens + 1
    color_part = color_len_full * needs_color_change.to(torch.int64)
...
def ansi_generate_256_quadrant(
    xs: torch.Tensor,
    ys: torch.Tensor,
    styles: torch.Tensor,
    byte_vals: torch.Tensor,
    ...
    """Generate 256-color ANSI for quadrant mode.

    `styles` is (N, 3) uint8: [fg_idx, bg_idx, glyph_idx].
    """
    N = xs.numel()
    if N == 0:
        return torch.tensor(RESET_VALS, dtype=torch.uint8, device=cfg.device)
    ...
    # 256-color: fg = \e[38;5;Nm  bg = \e[48;5;Nm
    fg_len_full = 7 + fg_lens + 1
    bg_len_full = 7 + bg_lens + 1
    style_part = (fg_len_full + bg_len_full) * needs_style_change.to(torch.int64)
