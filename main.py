...
def writer_task(out_queue: queue.Queue, target_fps: float) -> None:
...
def ansi_generate(xs: torch.Tensor, ys: torch.Tensor, colors: torch.Tensor, byte_vals: torch.Tensor, byte_lens: torch.Tensor, cfg: Config) -> torch.Tensor:
    ...
    per_pixel_len = (2 + row_lens + 1 + col_lens + 1 + 7 + r_lens + 1 + g_lens + 1 + b_lens + 2)
    total_len = per_pixel_len.sum()
    offsets = torch.cat((torch.zeros(1, dtype=torch.int64, device=device), per_pixel_len.cumsum(dim=0)[:-1]), dim=0)
...
def frame_producer(path: Optional[str], capture_screen: bool, cfg: Config,
                   ...
                   region: Optional[Tuple[int,int,int,int]], fps: int) -> None:
    prev_gray = None
    source = None
    if path:
...
def main() -> None:
    ...
    writer = threading.Thread(target=writer_task, args=(out_q, fps,), daemon=True)
    writer.start()
